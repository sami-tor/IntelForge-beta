...
export async function GET(request: NextRequest) {
  ...
  if (!validation.valid || !validation.sanitized) {
    return NextResponse.json(
      { error: validation.errors[0] || "Invalid query", results: [], sources: {} },
      { status: 400 }
    )
  }
  ...
  if (!isGuest) {
    ...
    if (!userResult.success || !userResult.data?.length) {
      return NextResponse.json({ error: "User not found", results: [], sources: {} }, { status: 404 })
    }
    ...
    if (!isAdmin && user.search_count >= user.search_limit) {
      return NextResponse.json(
        { error: "Search limit exceeded", searches_used: user.search_count, searches_limit: user.search_limit, results: [], sources: {} },
        { status: 429 }
      )
    }
  ...
  const clientLimit = Math.min(limitParam, maxResults)
  ...
  try {
    ...
    const pgLimit = Math.min(clientLimit, 100)

    // ─── SOURCE 1: Full-text index on search_index_lines (PostgreSQL) ───
    if (!guestOnly && (!type || type === "all" || type === "index" || type === "files")) {
      const indexResult = await dbQuery(
        `SELECT
         ...
         LIMIT $2`,
        [sanitizedQuery, pgLimit]
      ).catch(() => ({ success: false, data: [] }))

      if (indexResult.success && Array.isArray(indexResult.data) && indexResult.data.length > 0) {
        sources["index"] = indexResult.data.length
        for (const row of indexResult.data) {
          const displayPath = (row.file_path || "").replace(/\\/g, "/").replace(/^\/data\//, "")
          ...
          const plain = row.highlighted?.replace(/<\/?mark>/g, "") || row.content || ""
    ...
    if (!guestOnly && (!type || type === "all" || type === "darkweb" || type === "darkweb_market")) {
      const res = await dbQuery(
        `SELECT post_uid, source, source_type, title, content, url, threat_actor, victim_name,
         ...
         LIMIT $2`,
        [searchLike, Math.min(clientLimit, 50)]
      ).catch(() => ({ success: false, data: [] as any[] }))

      if (res.success && Array.isArray(res.data) && res.data.length > 0) {
        sources["darkweb"] = res.data.length
        const blurred = shouldBlur && !isAdmin
        for (const row of res.data) {
          const fullContent = [
            blurred ? BLURRED_DARK : row.content,
            row.threat_actor ? `Threat Actor: ${row.threat_actor}` : "",
            row.victim_name ? `Victim: ${row.victim_name}` : "",
            row.victim_sector ? `Sector: ${row.victim_sector}` : "",
            row.victim_country ? `Country: ${row.victim_country}` : "",
            row.severity ? `Severity: ${row.severity}` : "",
            row.leak_type ? `Leak Type: ${row.leak_type}` : "",
          ].filter(Boolean).join("\n")
          ...
          results.push({
            ...
            Content: fullContent,
            Preview: blurred
              ? "[Dark Web Intelligence - Premium Content]"
              : blur(row.content || row.title || "", 250),
            fileType: "darkweb",
            isBlurred: blurred,
    ...
    if (!type || type === "all" || type === "demo" || type === "cti") {
      const res = await dbQuery(
    ...
    if (!guestOnly && (!type || type === "all" || type === "stealer" || type === "stealer_logs")) {
      const res = await dbQuery(
      ...
      if (res.success && Array.isArray(res.data) && res.data.length > 0) {
        sources["stealer"] = res.data.length
        const blurred = shouldBlur && !isAdmin
        for (const row of res.data) {
          const fullContent = [
            blurred ? BLURRED_STEALER : `URL: ${row.captured_url}`,
            blurred ? "" : `Domain: ${row.domain}`,
            blurred ? "" : `Username: ${row.login_user}`,
            `Stealer: ${row.stealer_family}`,
          ...
          results.push({
            ...
            Content: fullContent,
            Preview: blurred
              ? "[Stealer Logs - Premium Content]"
              : `${row.stealer_family}: ${row.login_user} @ ${row.domain}`,
    ...
    // ─── SOURCE 5: CVE Cache ───
    if (!type || type === "all" || type === "cve" || type === "vulnerability") {
      const res = await dbQuery(
        `SELECT cve_id, description, cvss_v3_score, cvss_v3_severity, epss_score, is_kev,
                vendor, product, published_at, last_modified
         ...
         LIMIT $2`,
        [searchLike, Math.min(clientLimit, 30)]
      ).catch(() => ({ success: false, data: [] as any[] }))

      if (res.success && Array.isArray(res.data) && res.data.length > 0) {
        sources["cve"] = res.data.length
        for (const row of res.data) {
          const kevBadge = row.is_kev ? " [CISA KEV]" : ""
          const fullContent = [
            row.description,
            `CVSS: ${row.cvss_v3_score}/10 (${row.cvss_v3_severity})${kevBadge}`,
            `EPSS: ${row.epss_score}`,
            `Vendor: ${row.vendor}`,
            `Product: ${row.product}`,
    ...
    if (!type || type === "all" || type === "malware") {
      const res = await dbQuery(
        `SELECT sha256, file_name, file_type, malware_family, tags, source, iocs,
      ...
      if (res.success && Array.isArray(res.data) && res.data.length > 0) {
        sources["malware"] = res.data.length
        for (const row of res.data) {
          const tags = Array.isArray(row.tags) ? row.tags.join(", ") : ""
          const fullContent = [
            ...
            tags ? `Tags: ${tags}` : "",
            `Source: ${row.source}`,
            `SHA256: ${(row.sha256 || "").substring(0, 32)}...`,
    ...
    if (!type || type === "all" || type === "phishing") {
      const res = await dbQuery(
      ...
      if (res.success && Array.isArray(res.data) && res.data.length > 0) {
        sources["phishing"] = res.data.length
        const blurred = shouldBlur && !isAdmin
        for (const row of res.data) {
          const fullContent = [
            blurred ? BLURRED_PHISH : `URL: ${row.url}`,
            blurred ? "" : `Target Brand: ${row.target_brand}`,
            `Phish Type: ${row.phish_type}`,
            blurred ? "" : `IP: ${row.ip_address}`,
            `Country: ${row.country}`,
          ...
          results.push({
            ...
            Content: fullContent,
            Preview: blurred
              ? "[Phishing URL - Premium Content]"
              : `${row.target_brand}: ${row.url}`,
            fileType: "phishing",
            isBlurred: blurred,
    ...
    if (!guestOnly && (!type || type === "all" || type === "paste" || type === "pastebin")) {
      const res = await dbQuery(
        `SELECT post_uid, source, title, excerpt, indicator_kinds, matched_brands,
                matched_cves, threat_actor, severity, discovered_at
      ...
      if (res.success && Array.isArray(res.data) && res.data.length > 0) {
        sources["paste"] = res.data.length
        const blurred = shouldBlur && !isAdmin
        for (const row of res.data) {
          const brands = Array.isArray(row.matched_brands) ? row.matched_brands.join(", ") : ""
          const fullContent = [
            blurred ? BLURRED_PASTE : row.excerpt,
            brands ? `Brands: ${brands}` : "",
            row.threat_actor ? `Actor: ${row.threat_actor}` : "",
            `Severity: ${row.severity}`,
            `Source: ${row.source}`,
    ...
    if (!type || type === "all" || type === "actors" || type === "apt") {
      const res = await dbQuery(
        `SELECT stix_id, name, group_id, aliases, description, sectors, countries
         ...
         ORDER BY name ASC
         LIMIT $2`,
        [searchLike, Math.min(clientLimit, 20)]
      ).catch(() => ({ success: false, data: [] as any[] }))

      if (res.success && Array.isArray(res.data) && res.data.length > 0) {
        sources["actors"] = res.data.length
        for (const row of res.data) {
          const aliases = Array.isArray(row.aliases) ? row.aliases.join(", ") : ""
          const sectors = Array.isArray(row.sectors) ? row.sectors.join(", ") : ""
          const countries = Array.isArray(row.countries) ? row.countries.join(", ") : ""
          results.push({
            source: "actors",
            ...
            LineNum: 0,
            Content: [row.description, aliases ? `Aliases: ${aliases}` : "", sectors ? `Sectors: ${sectors}` : "", countries ? `Countries: ${countries}` : "", `STIX ID: ${row.stix_id}`, `Group ID: ${row.group_id}`].filter(Boolean).join("\n"),
            Preview: blur(row.description || "", 250),
            fileType: "actor",
            isBlurred: false,
    ...
    if (!type || type === "all" || type === "ransomware" || type === "victims") {
      const res = await dbQuery(
        `SELECT victim_uid, group_name, victim_name, victim_sector, victim_country,
         ...
         WHERE victim_name ILIKE $1 OR group_name ILIKE $1 OR victim_sector ILIKE $1
      ...
      if (res.success && Array.isArray(res.data) && res.data.length > 0) {
        sources["ransomware"] = res.data.length
        const blurred = shouldBlur && !isAdmin
        for (const row of res.data) {
          results.push({
            source: "ransomware",
            ...
            LineNum: 0,
            Content: [
              blurred ? "[Ransomware Intel - Premium]" : row.description,
              `Group: ${row.group_name}`,
              `Sector: ${row.victim_sector}`,
              `Country: ${row.victim_country}`,
              row.ransom_amount ? `Ransom: ${row.ransom_amount}` : "",
              `Status: ${row.status}`,
              `Discovered: ${row.discovered_at}`,
            ].filter(Boolean).join("\n"),
    ...
    if (sessionUser && !isAdmin) {
      await dbQuery(
        `UPDATE users SET search_count = search_count + 1
         WHERE id = $1 AND search_count < search_limit`,
        [sessionUser.id]
      ).catch(() => {})

      const clientIp = request.headers.get("x-forwarded-for")?.split(",")[0].trim() || "unknown"
      await dbQuery(
        `INSERT INTO search_history (user_id, query, search_type, results_count, ip_address, created_at)
         VALUES ($1, $2, 'web', $3, $4, NOW())`,
        [sessionUser.id, sanitizedQuery, results.length, clientIp]
      ).catch(() => {})
    }
    ...
    return NextResponse.json({
      ...
      results: results.slice(0, clientLimit),
      sources,
    ...
    return NextResponse.json(
      { error: error.message || "Search failed", results: [], sources: {} },
      { status: 500 }
    )
  }
}
