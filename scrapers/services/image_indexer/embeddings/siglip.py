#!/usr/bin/env python3
"""
SigLIP Embedding Generator (768-dim) with memory management
"""
...
class SigLIPEmbedder:
    """SigLIP model for semantic image embeddings with memory management"""
    
    def __init__(self):
        self.processor = None
        self.model = None
        self.device = "cuda" if torch.cuda.is_available() else "cpu"
        self.model_key = "siglip_embedder"
        self._load_model()
    
    def _load_model(self):
        """Load SigLIP model with memory management"""
        try:
            # Check if model is already loaded in memory manager
            existing_model = memory_manager.get_model(self.model_key)
            if existing_model:
                logger.info("Using existing SigLIP model from memory manager")
                self.processor = existing_model.processor
                self.model = existing_model.model
                self.device = existing_model.device
                return
            
            logger.info("[INFO] Loading SigLIP model...")
            ...
            memory_manager.register_model(self.model_key, self, "siglip")
            
            logger.info(f"[OK] SigLIP model loaded on {self.device}")
        except Exception as e:
            logger.error(f"[WARN] Failed to load SigLIP: {e}")
            self.processor = None
            self.model = None
    ...
    def generate(self, image: Image.Image) -> Optional[torch.Tensor]:
        """
        Generate SigLIP embedding (768-dim)
        Returns normalized embedding tensor
        """
        if not self.is_available():
            logger.warning("SigLIP model not available")
            return None
        ...
        try:
            ...
            logger.debug(f"Generated SigLIP embedding: {result.shape}")
            return result
            
        except Exception as e:
            logger.error(f"[ERROR] Error generating SigLIP embedding: {e}")
    ...
    def generate_batch(self, images: list) -> list:
        ...
        if not self.is_available():
            logger.warning("SigLIP model not available for batch processing")
            return []
        ...
        try:
            ...
            logger.debug(f"Generated {len(results)} SigLIP embeddings from {len(valid_images)} images")
            return results
            
        except Exception as e:
            logger.error(f"[ERROR] Error generating SigLIP batch embeddings: {e}")
            raise  # Let decorator handle retry and fallback
    ...
    def cleanup(self):
        ...
        try:
            ...
            self.processor = None
            self.model = None
            logger.info("SigLIP model cleaned up")
        except Exception as e:
            logger.error(f"Error cleaning up SigLIP model: {e}")
